...
const filterLabels = ["Ressource", "Famille", "Compétence", "Niveau", "Criticité", "Statut", "Manager", "Service"];
const kpiLabels = ["Compétences suivies", "Écarts critiques", "Experts disponibles", "Plans actifs"];
...
async function loadPageData(slugOrId: string): Promise<{ organization: Organization; rows: EnrichedModuleRow[] }> {
  ...
  const [rowsResult, employeesResult] = await Promise.all([
    (supabase.from("hr_employee_skills" as never) as any)
      .select("*")
...
export default function Page({ params }: { params: Promise<PageParams> }) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["rh-module", "competences", orgId],
  ...
  if (isLoading) {
    return <div className="p-6 text-sm font-semibold text-slate-500">Chargement du module Compétences…</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Compétences"
        subtitle="Cartographie des compétences, niveaux cibles, écarts critiques, plans de développement et staffing par expertise."
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" className="inline-flex h-9 items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-3.5 text-xs font-bold text-white shadow-md shadow-indigo-100 transition hover:-translate-y-0.5 hover:shadow-lg dark:shadow-none">
              <Plus className="h-3.5 w-3.5" /> Nouvelle compétence
            </button>
          </div>
        }
      />
      ...
      <PageTutorial
        title="Piloter les compétences"
        description="Identifier les expertises disponibles, les écarts de niveau, les dépendances critiques et les plans de montée en compétence."
        objectives={[
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="text-base font-black text-slate-950 dark:text-white">Synthèse métier</h2><p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">Un manager, responsable RH ou chef de projet expert attend une matrice compétence/niveau, la disponibilité des experts, les risques de dépendance et le lien direct avec staffing, entretiens et formation.</p></div>
          </div>
        </article>
