...
const filterLabels = ["Ressource", "Manager", "Recruteur", "Statut", "Risque", "Période", "Site", "Service"];
const kpiLabels = ["Plans actifs", "À préparer", "À risque", "Terminés"];
...
export default function Page({ params }: { params: Promise<PageParams> }) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Onboarding"
        subtitle="Préparation des arrivées, parcours d’intégration, tâches RH/manager/recrutement, risques et progression."
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" className="inline-flex h-9 items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-3.5 text-xs font-bold text-white shadow-md shadow-indigo-100 transition hover:-translate-y-0.5 hover:shadow-lg dark:shadow-none">
              <Plus className="h-3.5 w-3.5" /> Nouvel onboarding
            </button>
          </div>
        }
      />
      ...
      <PageTutorial
        title="Piloter les intégrations"
        description="Sécuriser les arrivées avec une checklist partagée RH, manager, recrutement, IT et finance."
        objectives={[
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="text-base font-black text-slate-950 dark:text-white">Synthèse métier</h2><p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">Un responsable recrutement attend un parcours cadencé avant/après arrivée, des alertes sur les blocages, une responsabilité claire par tâche et une traçabilité complète.</p></div>
          </div>
        </article>
