...
const filters = ["Ressource", "Manager", "Recruteur", "Statut", "Risque", "Période"];
const kpiLabels = ["Parcours actifs", "Arrivées à risque", "Tâches en retard", "Parcours finalisés"];
...
export default function Page({ params }: { params: Promise<PageParams> }) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["rh-module", "onboarding", orgId],
  ...
  if (isLoading) {
    return <div className="p-6 text-sm font-semibold text-slate-500">Chargement du module Onboarding…</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Onboarding"
        subtitle="Préparation, suivi et sécurisation des arrivées : tâches, jalons, risques, responsables et complétude dossier."
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3"><div className="rounded-xl bg-emerald-100 p-2.5 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300"><BarChart3 className="h-4 w-4" /></div><div><h2 className="text-base font-black text-slate-950 dark:text-white">Attendus expert</h2><p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">Un responsable recrutement attend un parcours standardisé mais adaptable, des alertes avant arrivée, la coordination RH/manager/IT, la mesure du délai de productivité et la traçabilité des validations.</p></div></div>
