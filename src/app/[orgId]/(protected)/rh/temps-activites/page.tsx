...
const filterLabels = ["Ressource", "Projet", "Activité", "Statut", "Période", "Manager", "Site", "Service"];
const kpiLabels = ["Temps saisis", "À valider", "Validés", "Archivés"];
...
export default function Page({ params }: { params: Promise<PageParams> }) {
  ...
  if (isLoading) {
    return <div className="p-6 text-sm font-semibold text-slate-500">Chargement du module Temps et activités…</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Saisie des temps, activités projets, validations manager, écarts capacité/réel et exports RH/finance."
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" className="inline-flex h-9 items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-3.5 text-xs font-bold text-white shadow-md shadow-indigo-100 transition hover:-translate-y-0.5 hover:shadow-lg dark:shadow-none">
              <Plus className="h-3.5 w-3.5" /> Nouveau temps
            </button>
          </div>
        }
      />
      ...
      <PageTutorial
        title="Piloter les temps et activités"
        description="Suivre le temps saisi, les validations, les écarts et la contribution aux projets sans ressaisie entre RH, staffing et finance."
        objectives={[
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="text-base font-black text-slate-950 dark:text-white">Synthèse métier</h2><p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">Un expert RH et chef de projet attend une vision fiable des heures saisies, des écarts entre capacité et réalisé, des validations manager et des imputations projet exploitables par la finance.</p></div>
          </div>
        </article>
