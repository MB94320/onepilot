...
type HrAbsenceFiltersProps = {
  ...
  onChange: (value: HrAbsenceFiltersValue) => void;
  resultCount: number;
};
...
function uniqueValues(
  requests: HrAbsenceFilterRequest[],
  getValue: (request: HrAbsenceFilterRequest) => string | null | undefined,
) {
  return Array.from(
    new Set(
      requests
...
export default function HrAbsenceFilters({
  ...
  const absenceTypes = uniqueValues(
    requests,
    (request) => request.absence_type_name,
  );

  const sites = uniqueValues(
    requests,
    (request) => request.site_name,
  );
  ...
  function updateFilter<K extends keyof HrAbsenceFiltersValue>(
    field: K,
    fieldValue: HrAbsenceFiltersValue[K],
  ) {
    onChange({
      ...value,
      [field]: fieldValue,
      ...
      <div className="border-b border-slate-100 bg-gradient-to-r from-sky-50/70 via-white to-indigo-50/60 px-5 py-4 dark:border-slate-800 dark:from-sky-950/20 dark:via-slate-950 dark:to-indigo-950/20">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Les filtres pilotent les KPI, les analyses, les graphiques, les validations, les soldes et les exports.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-full border border-indigo-100 bg-white px-3 py-1.5 text-xs font-semibold text-indigo-700 shadow-sm dark:border-indigo-900 dark:bg-slate-950 dark:text-indigo-300">
            {resultCount} résultat{resultCount > 1 ? "s" : ""} sur {requests.length}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 p-5">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Rechercher un collaborateur, un matricule, un type d’absence, un site, un service ou un manager..."
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          >
            ...
            <option value="manager_approved">Validée manager</option>
          ...
          <select
            value={value.archive}
            onChange={(event) => updateFilter("archive", event.target.value)}
            className={selectClassName}
          >
            <option value="active">Demandes actives</option>
            <option value="archived">Demandes archivées</option>
            <option value="all">Actives et archivées</option>
          </select>
        </div>
