...
import { BookOpen, Layers3, Save, Search, X } from "lucide-react";
...
export default function HrEmployeeSkillsForm({ organizationId, employees, catalog, assessments, initialEmployeeId, onClose, onSaved }: { organizationId: string; employees: AnyRow[]; catalog: AnyRow[]; assessments: AnyRow[]; initialEmployeeId?: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-[2px]" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><section className="max-h-[94vh] w-full max-w-[1440px] overflow-y-auto rounded-2xl border border-slate-200 bg-slate-50 shadow-2xl dark:border-slate-700 dark:bg-slate-900" onMouseDown={(event) => event.stopPropagation()}><div className="sticky top-0 z-[70] flex items-start gap-4 border-b border-slate-200 bg-gradient-to-r from-sky-50 via-white to-indigo-50 px-5 py-4 dark:border-slate-700 dark:from-sky-900/30 dark:via-slate-800 dark:to-indigo-900/30"><div className="flex gap-3"><span className="rounded-xl bg-indigo-100 p-2.5 text-indigo-700"><BookOpen className="h-5 w-5" /></span><div><h2 className="text-xl font-black text-slate-950 dark:text-white">Formulaire compétences</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-300">Évaluez progressivement la ressource par chapitre et sous-chapitre. Toute compétence non renseignée conserve le niveau 0.</p></div></div></div><div className="p-5">
      ...
      <div className="max-h-[520px] overflow-auto">
        <table className="w-full min-w-[1480px] border-separate border-spacing-0">
          <thead className="sticky top-0 z-40">
            <tr>
              <th className="sticky left-0 z-50 w-[140px] min-w-[140px] max-w-[140px] bg-sky-50 text-left">Chapitre</th>
              <th className="sticky left-[140px] z-50 w-[170px] min-w-[170px] max-w-[170px] bg-sky-50 text-left">Sous-chapitre</th>
              <th className="sticky left-[310px] z-50 w-[240px] min-w-[240px] max-w-[240px] bg-sky-50 text-left shadow-[8px_0_12px_-12px_rgba(15,23,42,.8)]"><HrColumnFilterMenu label="Compétence" values={scope.map((skill) => String(skill.name || "—"))} selected={skillFilters.skill || []} onChange={(values) => setSkillFilters((current) => ({ ...current, skill: values }))} /></th>
              <th className="min-w-[440px] text-left">Attendus des niveaux 0 à 4</th>
          ...
          <tbody>{filtered.map((skill) => {
            ...
            return <tr key={skill.id}>
              <td className="sticky left-0 z-20 w-[140px] min-w-[140px] max-w-[140px] truncate bg-white dark:bg-slate-800" title={alpha(skill.family)}>{alpha(skill.family)}</td>
              <td className="sticky left-[140px] z-20 w-[170px] min-w-[170px] max-w-[170px] truncate bg-white dark:bg-slate-800" title={alpha(skill.category)}>{alpha(skill.category)}</td>
              <td className="sticky left-[310px] z-20 w-[240px] min-w-[240px] max-w-[240px] truncate bg-white font-semibold text-slate-800 shadow-[8px_0_12px_-12px_rgba(15,23,42,.8)] dark:bg-slate-800 dark:text-slate-100" title={skill.name}>{skill.name}</td>
              <td className="min-w-[440px]"><div className="grid gap-1">{levels.map((value) => <div key={value} className="flex items-start gap-2 text-[11px]"><span className={`inline-flex min-w-7 justify-center rounded-full px-2 py-0.5 font-black ${levelClasses[value]}`}>{value}</span><span className="leading-4 text-slate-600 dark:text-slate-300">{expected(skill, value)}</span></div>)}</div></td>
    ...
    {error && <p className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-700">{error}</p>}<div className="sticky bottom-0 mt-6 flex flex-wrap justify-end gap-3 border-t border-slate-200 bg-white/95 px-1 py-4 backdrop-blur dark:border-slate-700 dark:bg-slate-900/95"><button type="button" onClick={onClose} className={hrCancelButtonClassName}>Annuler</button><button type="button" disabled={saving || !employeeId || !scope.length} onClick={() => void save()} className={hrSaveButtonClassName}><Save className="h-4 w-4" />{saving ? "Enregistrement…" : "Enregistrer ce sous-chapitre"}</button></div></div></section></div>;
}
