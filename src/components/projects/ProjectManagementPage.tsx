...
function enrich(data: ProjectData) {
  ...
  const projects: AnyRow[] = data.projects.map((row) => {
    ...
    return {
      ...
      action_count: data.actions.filter((action) => action.project_id === row.id && !action.archived_at).length,
      risk_count: data.risks.filter((risk) => risk.project_id === row.id && !risk.archived_at).length,
    };
  });
...
function rowTitle(mode: ProjectPageMode, row: AnyRow) {
  ...
  if (mode === "actions") return `${row.code} · ${row.title || row.name}`;
...
function ProjectCard({ mode, row, onView, onEdit, onArchive, onRestore }: { mode: ProjectPageMode; row: AnyRow; onView: () => void; onEdit: () => void; onArchive: () => void; onRestore: () => void }) {
  ...
  const info: Array<[string, ReactNode, any]> = mode === "portfolio"
    ...
    : mode === "actions"
      ? [["Projet rattaché", `${row.project_code || "NA"} · ${row.project_name || "Action transverse"}`, "sky"], ["Origine", originLabel(row.origin_type || row.action_type), "sky"], ["Responsable", row.owner_name, "indigo"], ["Échéance", formatDate(row.replanned_due_date || row.due_date), "amber"], ["Avancement", percent(row.progress_percent), "emerald"]]
      : mode === "performance"
...
function ActionComplementFields({ values, set }: { values: Record<string, string>; set: (key: string, value: string) => void }) {
  return <>
    <label><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Priorité</span><select value={values.priority} onChange={(event) => set("priority", event.target.value)} className={`${hrSelectClassName} mt-1 w-full`}><option value="low">Faible</option><option value="normal">Moyenne</option><option value="high">Haute</option><option value="critical">Critique</option></select></label>
    <label><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Type d’action</span><select value={values.action_category} onChange={(event) => set("action_category", event.target.value)} className={`${hrSelectClassName} mt-1 w-full`}><option value="corrective">Corrective</option><option value="preventive">Préventive</option><option value="improvement">Amélioration</option><option value="decision">Décision</option></select></label>
    <label className="sm:col-span-2"><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Contributeurs</span><input value={values.contributors_text} onChange={(event) => set("contributors_text", event.target.value)} placeholder="Noms des contributeurs" className={`${hrInputClassName} mt-1 w-full`} /></label>
    <label><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Date de fin réelle</span><input type="date" value={values.actual_completion_date} onChange={(event) => set("actual_completion_date", event.target.value)} className={`${hrInputClassName} mt-1 w-full`} /></label>
    <label><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Efficacité</span><select value={values.effectiveness_status} onChange={(event) => set("effectiveness_status", event.target.value)} className={`${hrSelectClassName} mt-1 w-full`}><option value="">Non évaluée</option><option value="compliant">Conforme</option><option value="partially_compliant">Partiellement conforme</option><option value="non_compliant">Non conforme</option></select></label>
    <label className="sm:col-span-2"><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Résultat réel</span><input value={values.actual_result} onChange={(event) => set("actual_result", event.target.value)} className={`${hrInputClassName} mt-1 w-full`} /></label>
    <label className="sm:col-span-2"><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Commentaire d’efficacité</span><input value={values.effectiveness_comment} onChange={(event) => set("effectiveness_comment", event.target.value)} className={`${hrInputClassName} mt-1 w-full`} /></label>
    <label className="sm:col-span-2"><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Commentaire de clôture</span><input value={values.closure_comment} onChange={(event) => set("closure_comment", event.target.value)} className={`${hrInputClassName} mt-1 w-full`} /></label>
  </>;
}
...
function Form({ mode, data, row, onClose, onSaved }: { mode: ProjectPageMode; data: ProjectData; row?: AnyRow | null; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useState<Record<string, string>>({
    ...
    action_category: row?.action_category || row?.action_type || "corrective", contributors_text: row?.contributors_text || "", actual_completion_date: row?.actual_completion_date || "",
    actual_result: row?.actual_result || "", effectiveness_comment: row?.effectiveness_comment || "", closure_comment: row?.closure_comment || "",
  });
  ...
  async function save() {
    ...
    try {
      ...
      else if (mode === "actions") { table = "project_actions"; payload = { organization_id: data.organization.id, project_id: values.project_id || null, code: await generatedCode("ACT"), title: values.name.trim(), description: values.description || null, status: values.status, opened_at: values.start_date, due_date: values.end_date || null, replanned_due_date: values.replanned_due_date || null, actual_completion_date: values.actual_completion_date || null, owner_employee_id: values.manager_id || null, owner_name: person(data.employees.find((item) => item.id === values.manager_id)), priority: values.priority, action_category: values.action_category, contributors_text: values.contributors_text || null, origin_type: values.origin, origin_reference: values.origin_reference || null, expected_result: values.expected_result || null, actual_result: values.actual_result || null, impact: values.impact || null, root_cause: values.root_cause || null, proof_reference: values.proof_reference || null, effectiveness_status: values.effectiveness_status || null, effectiveness_comment: values.effectiveness_comment || null, effectiveness_review_date: values.effectiveness_review_date || null, closure_comment: values.closure_comment || null, progress_percent: number(values.amount), comments: values.description || null, generic_reason: values.project_id ? null : values.description || "Action transverse", updated_at: new Date().toISOString() }; }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-600 dark:bg-slate-800"><div className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-sky-50 via-white to-indigo-50 px-5 py-4 dark:border-slate-600 dark:from-sky-900/25 dark:via-slate-800 dark:to-indigo-900/25"><div><h2 className="text-sm font-black text-slate-950 dark:text-white">{row ? "Modifier" : "Nouveau"} {config[mode].entity}</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-300">Formulaire métier relié aux projets, clients, ressources, budgets et objets transverses.</p></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-white dark:hover:bg-slate-700"><X className="h-4 w-4" /></button></div><div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
    ...
    {mode === "actions" && <ActionComplementFields values={values} set={set} />}
  ...
  </div><div className="flex flex-col-reverse gap-3 border-t border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-end dark:border-slate-600"><button type="button" onClick={onClose} className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 transition hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300 dark:hover:bg-slate-900">Annuler</button><button type="button" onClick={() => void save()} disabled={saving || (mode !== "performance" && !values.name.trim())} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-4 text-sm font-bold text-white shadow-md shadow-indigo-100 transition hover:-translate-y-0.5 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50 dark:shadow-none">{saving ? "Enregistrement…" : "Enregistrer"}</button></div></div></div>;
}
...
export default function ProjectManagementPage({ params, mode }: { params: Promise<PageParams>; mode: ProjectPageMode }) {
  ...
  const analyticsData = { projects: projectScope, tasks: data.tasks, milestones: data.milestones, actions: data.actions, health: data.health, deliverables: data.deliverables, risks: data.risks, nonconformities: data.nonconformities, financials: data.financials, satisfaction: data.satisfaction, assignments: [...data.staffingAssignments, ...data.taskAssignments], employees: data.employees };
  const tabs: Array<{ key: TabKey; label: string; icon: ComponentType<{ className?: string }>; active: string }> = mode === "gantt" ? [
    ...
    { key: "gantt", label: "Gantt", icon: CalendarDays, active: "bg-violet-600 text-white" },
    { key: "critical", label: "Chemin critique", icon: GitBranch, active: "bg-emerald-600 text-white" },
  ...
  return <div className="space-y-6">
    ...
    {tab === "gantt" && <div className="space-y-5">{projectScope.map((project) => { const projectTasks = rows.filter((task) => task.project_id === project.id); if (!projectTasks.length) return null; const taskIds = new Set(projectTasks.map((task) => String(task.id))); const projectDependencies = data.dependencies.filter((link) => taskIds.has(String(link.predecessor_task_id)) && taskIds.has(String(link.successor_task_id))); return <ProjectGanttBoard key={project.id} tasks={projectTasks} dependencies={projectDependencies} employeeMap={enriched.employeeMap} onEditTask={setTaskEditing} onArchiveTask={(row) => archive.mutate({ row, restore: false })} onRestoreTask={(row) => archive.mutate({ row, restore: true })} />; })}</div>}
    {tab === "critical" && <div className="space-y-5">{projectScope.map((project) => { const projectTasks = rows.filter((task) => task.project_id === project.id); if (!projectTasks.length) return null; const taskIds = new Set(projectTasks.map((task) => String(task.id))); const projectDependencies = data.dependencies.filter((link) => taskIds.has(String(link.predecessor_task_id)) && taskIds.has(String(link.successor_task_id))); return <ProjectPertBoard key={project.id} tasks={projectTasks} dependencies={projectDependencies} />; })}</div>}
