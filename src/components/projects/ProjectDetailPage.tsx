...
function ProjectTable({ title, description, icon: Icon = ListChecks, columns, rows }: { title?: string; description?: string; icon?: ComponentType<{ className?: string }>; columns: Array<{ label: string; value: (row: AnyRow) => any }>; rows: AnyRow[] }) {
  ...
  const visibleRows = rows.filter((row) => columns.every((column, index) => {
    const filter = columnFilters[index]?.trim().toLocaleLowerCase("fr");
    if (!filter) return true;
    const value = column.value(row);
    return (typeof value === "string" || typeof value === "number") && String(value).toLocaleLowerCase("fr").includes(filter);
  }));
    ...
    <section ref={captureRef} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm fullscreen:overflow-auto dark:border-slate-600 dark:bg-slate-700/70">
      ...
      <table className="w-full min-w-[1280px] border-separate border-spacing-0 bg-white text-sm dark:bg-slate-700">
        <thead className="sticky top-0 z-20 bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-600 dark:text-slate-200">
          ...
          <tr>{columns.map((column, index) => <th key={`filter-${column.label}`} className={`${index === 0 ? "sticky left-0 z-30" : ""} bg-sky-50 px-2 py-2`}><input aria-label={`Filtrer ${column.label}`} value={columnFilters[index] || ""} onChange={(event) => setColumnFilters((current) => ({ ...current, [index]: event.target.value }))} placeholder="Filtrer" className="h-8 min-w-28 rounded-lg border border-slate-200 bg-white px-2 text-[11px] font-medium normal-case tracking-normal text-slate-600 outline-none focus:border-indigo-300" /></th>)}<th className="sticky right-0 z-30 bg-sky-50" /></tr>
        </thead>
