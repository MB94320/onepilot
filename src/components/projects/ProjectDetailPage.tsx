...
import { use, useMemo, useRef, useState, type ComponentType } from "react";
...
async function loadProject(orgId: string, projectId: string) {
  ...
  const skillLibraryResult = await table("hr_skills");
...
function ProjectTable({ columns, rows }: { columns: Array<{ label: string; value: (row: AnyRow) => any }>; rows: AnyRow[] }) {
  ...
  return (
    <div className="space-y-2"><div className="flex justify-end gap-2"><DataExportMenu data={rows} columns={exportColumns} fileName="onepilot_detail_projet" sheetName="Détail projet" disabled={!rows.length} /><ProjectVisualActions targetRef={captureRef} fileName="onepilot-detail-projet" label="le tableau" /></div><div ref={captureRef} className="max-h-[252px] overflow-auto rounded-2xl border border-slate-200 dark:border-slate-600">
...
export default function ProjectDetailPage({ params }: { params: Promise<Params> }) {
  ...
  const skillChapters: string[] = [...new Set<string>(data.skills.map((row: AnyRow) => String(row.skill_family || "Général").split("/")[0].trim()))].sort((a, b) => a.localeCompare(b, "fr"));
  const skillSubchapters: string[] = [...new Set<string>(data.skills.filter((row: AnyRow) => !skillChapter || String(row.skill_family || "Général").startsWith(skillChapter)).map((row: AnyRow) => String(row.skill_family || "Général / Compétences").split("/")[1]?.trim() || "Compétences"))].sort((a, b) => a.localeCompare(b, "fr"));
  const scopedSkills = data.skills.filter((row: AnyRow) => { const [chapter, subchapter = "Compétences"] = String(row.skill_family || "Général / Compétences").split("/").map((value: string) => value.trim()); return (!skillChapter || chapter === skillChapter) && (!skillSubchapter || subchapter === skillSubchapter); });
  const skillRadar = scopedSkills.slice().sort((left: AnyRow, right: AnyRow) => String(left.skill_code || "").localeCompare(String(right.skill_code || ""), "fr", { numeric: true })).slice(0, 12).map((row: AnyRow) => ({
    name: row.skill_name,
  ...
  return <div className="space-y-6">
    ...
    {tab === "quality" && <HrSectionCard icon={ClipboardCheck} title="Référentiel et résultats détaillés de l’audit" description="Questions applicables, conformité, commentaires et preuves, classés selon les thèmes AVV et Delivery."><ProjectTable rows={data.auditResponses.map((response: AnyRow) => { const question = data.auditQuestions.find((item: AnyRow) => item.id === response.question_id) || {}; const theme = data.auditThemes.find((item: AnyRow) => item.id === response.theme_id) || {}; return { ...response, question_code: question.code, question_text: question.question_text, theme_name: theme.name }; })} columns={[{ label: "Thème", value: (row) => row.theme_name }, { label: "Référence", value: (row) => row.question_code }, { label: "Question de contrôle", value: (row) => row.question_text }, { label: "Résultat", value: (row) => <HrStatusBadge status={row.answer === "yes" ? "completed" : row.answer === "no" ? "blocked" : "archived"} label={row.answer === "yes" ? "Conforme" : row.answer === "no" ? "Non conforme" : "Non applicable"} /> }, { label: "Score", value: (row) => row.score == null ? "—" : row.score + " %" }, { label: "Commentaire / preuve", value: (row) => row.comment || row.evidence_reference || "—" }]} /></HrSectionCard>}
    ...
    {skillsEditing && <ProjectSkillForm organizationId={data.organization.id} projectId={project.id} library={data.skillLibrary} initialRows={data.skills} onClose={() => setSkillsEditing(false)} onSaved={() => void query.refetch()} />}
  </div>;
}
