...
type TabKey = "cockpit" | "planning" | "team" | "quality" | "finance" | "audit";
    ...
    <div className="grid grid-cols-[110px_repeat(4,minmax(70px,1fr))] gap-1 text-center text-[11px] font-black">
      ...
      {[4, 3, 2, 1].flatMap((probability) => [
        ...
        ...[1, 2, 3, 4].map((impact) => {
          ...
          return <div key={`${probability}-${impact}`} className={`min-h-16 rounded-lg p-2 ${tone}`} title={items.map((item) => `${item.code} · ${item.title}`).join("\n")}><div>{score}/16</div><div className="mt-1 text-lg">{items.length}</div></div>;
        }),
      ])}
    </div>
  );
...
export default function ProjectDetailPage({ params }: { params: Promise<Params> }) {
  ...
  const tabs: Array<{ key: TabKey; label: string; icon: ComponentType<{ className?: string }>; active: string }> = [
    ...
    { key: "quality", label: "Livrables, risques & qualité", icon: ShieldAlert, active: "bg-rose-600 text-white" },
    { key: "finance", label: "Finance & performance", icon: CircleDollarSign, active: "bg-sky-600 text-white" },
    { key: "audit", label: "Audit", icon: ClipboardCheck, active: "bg-slate-600 text-white" },
  ];
  return <div className="space-y-6">
    ...
    {tab === "planning" && <HrSectionCard icon={CalendarDays} title="Planification intégrée" description="WBS, tâches, jalons, dépendances, charge, avancement physique, chemin critique et aujourd’hui."><ProjectGanttBoard tasks={data.tasks} dependencies={data.dependencies} employeeMap={data.employeeMap} /></HrSectionCard>}
    ...
    {tab === "audit" && <HrSectionCard icon={ClipboardCheck} title="Audit et traçabilité projet" description="Créations, modifications, archivages et décisions sensibles historisés par organisation."><ProjectTable rows={data.audit} columns={[{ label: "Date", value: (row) => new Intl.DateTimeFormat("fr-FR", { dateStyle: "short", timeStyle: "short" }).format(new Date(row.created_at)) }, { label: "Objet", value: (row) => row.entity_table }, { label: "Action", value: (row) => row.action }, { label: "Utilisateur", value: (row) => row.actor_user_id || "Système" }, { label: "Motif", value: (row) => row.reason || "—" }]} /></HrSectionCard>}
  </div>;
}
