...
import { use, useMemo, useRef, useState, type ComponentProps, type ComponentType } from "react";
...
function ProjectTable({ title, description, icon: Icon = ListChecks, columns, rows }: { title?: string; description?: string; icon?: ComponentType<{ className?: string }>; columns: Array<{ label: string; value: (row: AnyRow) => any }>; rows: AnyRow[] }) {
  const captureRef = useRef<HTMLElement | null>(null);
  const exportColumns: ExportColumn<AnyRow>[] = columns.map((column, index) => ({ key: `column_${index}`, label: column.label, value: column.value }));
  return (
    <section ref={captureRef} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm fullscreen:overflow-auto dark:border-slate-600 dark:bg-slate-700/70">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-gradient-to-r from-sky-50/80 via-white to-indigo-50/70 px-4 py-3 dark:border-slate-600 dark:from-sky-900/20 dark:via-slate-700 dark:to-indigo-900/20"><div className="flex min-w-0 items-center gap-3"><span className="rounded-xl bg-sky-100 p-2.5 text-sky-700 dark:bg-sky-900/45 dark:text-sky-200"><Icon className="h-4 w-4" /></span><div className="min-w-0"><h3 className="text-sm font-black text-slate-950 dark:text-white">{title || "Détail du projet"}</h3><p className="mt-1 truncate text-xs text-slate-500 dark:text-slate-300">{description || "Données détaillées, filtrées et traçables du projet."}</p></div></div><div className="flex items-center gap-2"><DataExportMenu data={rows} columns={exportColumns} fileName="onepilot_detail_projet" sheetName={title || "Détail projet"} disabled={!rows.length} /><ProjectVisualActions targetRef={captureRef} fileName="onepilot-detail-projet" label="le tableau" /></div></div>
...
function RiskMatrix({ risks }: { risks: AnyRow[] }) {
  ...
  return (
    ...
    </div><div className="flex flex-wrap gap-4 border-t border-slate-100 px-5 py-3 text-[10px] font-black dark:border-slate-600"><span className="text-emerald-700">Vert · faible</span><span className="text-amber-700">Jaune · modéré</span><span className="text-orange-700">Orange · élevé</span><span className="text-rose-700">Rouge · critique</span></div></section>
  );
}
...
export default function ProjectDetailPage({ params }: { params: Promise<Params> }) {
  ...
  const scopedSkills = data.skills.filter((row: AnyRow) => { const meta = skillMeta(row); return (!skillChapter || meta.chapter === skillChapter) && (!skillSubchapter || meta.subchapter === skillSubchapter) && (!skillLevel || Number(row.required_level) === Number(skillLevel)) && (!skillImportance || row.importance === skillImportance); });
  ...
  return <div className="space-y-6">
    ...
    {tab === "quality" && <div className="space-y-5"><RiskMatrix risks={data.risks} /><ProjectTable title="Livrables et critères d’acceptation" description="Baseline, replanification, livraison, acceptation, preuve et bon du premier coup." icon={FileCheck2} rows={data.deliverables} columns={[{ label: "Livrable", value: (row) => `${row.code} · ${row.name}` }, { label: "Planifié", value: (row) => formatDate(row.planned_date) }, { label: "Replanifié", value: (row) => formatDate(row.replanned_date) }, { label: "Livré", value: (row) => formatDate(row.actual_delivery_date) }, { label: "Statut", value: (row) => <HrStatusBadge status={row.status} /> }, { label: "Premier coup", value: (row) => row.first_time_right == null ? "—" : row.first_time_right ? "Oui" : "Non" }, { label: "Critères", value: (row) => row.acceptance_criteria || "—" }]} /><ProjectTable title="Registre des risques" description="Exposition, réponse, responsabilité, échéance, revue et tendance." icon={AlertTriangle} rows={data.risks} columns={[{ label: "Risque", value: (row) => `${row.code} · ${row.title}` }, { label: "P × I", value: (row) => `${row.probability} × ${row.impact} = ${row.inherent_score || row.probability * row.impact}` }, { label: "Exposition", value: (row) => money(Number(row.revenue_impact_amount || 0) + Number(row.cost_impact_amount || 0)) }, { label: "Stratégie", value: (row) => <BusinessBadge value={row.response_strategy} kind="strategy" /> }, { label: "Responsable", value: (row) => row.owner_name }, { label: "Prochaine revue", value: (row) => formatDate(row.review_date) }, { label: "Plan", value: (row) => row.mitigation_plan || "—" }]} /><ProjectTable title="Qualité et efficacité" description="Décisions, actions correctives, preuves et contrôle d’efficacité ISO 9001." icon={CheckCircle2} rows={data.actions.filter((row: AnyRow) => ["quality", "risk", "nonconformity", "audit"].includes(String(row.origin_type || row.source_module || row.action_type)))} columns={[{ label: "Action", value: (row) => `${row.code} · ${row.title}` }, { label: "Origine", value: (row) => row.origin_reference || row.source_reference || row.origin_type }, { label: "Résultat attendu", value: (row) => row.expected_result || "—" }, { label: "Efficacité", value: (row) => <BusinessBadge value={row.effectiveness_status || "pending"} kind="effectiveness" /> }, { label: "Résultat", value: (row) => row.actual_result || "À renseigner" }]} /></div>}
    {tab === "team" && <ProjectTable title="Écarts et plan de développement" description="Comparaison nominative du niveau réel de chaque ressource avec le besoin projet, preuves et plan d’action recommandé." icon={Target} rows={skillGapRows.filter((row: AnyRow) => row.gap > 0)} columns={[{ label: "Ressource", value: (row) => row.resource_name }, { label: "Chapitre", value: (row) => `${row.chapterCode} · ${row.chapter}` }, { label: "Sous-chapitre", value: (row) => `${row.subchapterCode} · ${row.subchapter}` }, { label: "Compétence", value: (row) => `${row.skillCode} · ${row.skill_name}` }, { label: "Niveau réel", value: (row) => `N${row.current_level} / 4` }, { label: "Besoin projet", value: (row) => `N${row.required_level} / 4` }, { label: "Écart", value: (row) => <HrStatusBadge status={row.gap >= 2 ? "blocked" : "in_progress"} label={`−${row.gap} niveau${row.gap > 1 ? "x" : ""}`} /> }, { label: "Preuve", value: (row) => row.evidence }, { label: "Plan recommandé", value: (row) => row.action }, { label: "Priorité", value: (row) => <BusinessBadge value={row.importance === "critical" ? "high" : row.gap >= 2 ? "high" : "medium"} kind="priority" /> }]} />}
    {tab === "quality" && <ProjectTable title="Audit et conformité projet" description="Statut des contrôles, décisions, preuves et traçabilité des mises à jour sensibles." icon={ClipboardCheck} rows={data.audit} columns={[{ label: "Date", value: (row) => new Intl.DateTimeFormat("fr-FR", { dateStyle: "short", timeStyle: "short" }).format(new Date(row.created_at)) }, { label: "Objet contrôlé", value: (row) => frenchValue(row.entity_table) }, { label: "Action", value: (row) => frenchValue(row.action) }, { label: "Utilisateur", value: (row) => row.actor_user_id || "Système" }, { label: "Motif / preuve", value: (row) => row.reason || row.business_context || "—" }]} />}
    {tab === "quality" && <ProjectTable title="Référentiel et résultats détaillés de l’audit" description="Questions applicables, conformité, commentaires et preuves, classés selon les thèmes AVV et Delivery." icon={ClipboardCheck} rows={data.auditResponses.map((response: AnyRow) => { const question = data.auditQuestions.find((item: AnyRow) => item.id === response.question_id) || {}; const theme = data.auditThemes.find((item: AnyRow) => item.id === response.theme_id) || {}; return { ...response, question_code: question.code, question_text: question.question_text, theme_name: theme.name }; })} columns={[{ label: "Thème", value: (row) => row.theme_name }, { label: "Référence", value: (row) => row.question_code }, { label: "Question de contrôle", value: (row) => row.question_text }, { label: "Résultat", value: (row) => <HrStatusBadge status={row.answer === "yes" ? "completed" : row.answer === "no" ? "blocked" : "archived"} label={row.answer === "yes" ? "Conforme" : row.answer === "no" ? "Non conforme" : "Non applicable"} /> }, { label: "Score", value: (row) => row.score == null ? "—" : row.score + " %" }, { label: "Commentaire / preuve", value: (row) => row.comment || row.evidence_reference || "—" }]} />}
    {tab === "finance" && <div className="grid gap-5 xl:grid-cols-2"><HrChartCard title="Valeur acquise et coûts cumulés" description="VP, VA et CR avec lecture CPI/SPI et prévision à terminaison." exportConfig={{ type: "line", data: financialSeries, nameKey: "month", series: [{ key: "pv", label: "VP", color: colors.indigo }, { key: "ev", label: "VA", color: colors.emerald }, { key: "ac", label: "CR", color: colors.rose }], unit: " €" }}><ResponsiveContainer width="100%" height={300}><LineChart data={financialSeries}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Legend /><Line dataKey="pv" name="VP" stroke={colors.indigo} strokeWidth={3} /><Line dataKey="ev" name="VA" stroke={colors.emerald} strokeWidth={3} /><Line dataKey="ac" name="CR" stroke={colors.rose} strokeWidth={3} /></LineChart></ResponsiveContainer></HrChartCard><HrChartCard title="Production et facturation" description="Production, facturé et encaissement mensuels pour piloter FAE, PCA et trésorerie." exportConfig={{ type: "bar", data: financialSeries, nameKey: "month", series: [{ key: "production", label: "Production", color: colors.emerald }, { key: "billed", label: "Facturé", color: colors.indigo }] }}><ResponsiveContainer width="100%" height={300}><BarChart data={financialSeries}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Legend /><Bar dataKey="production" name="Production" fill={colors.emerald} radius={[7, 7, 0, 0]} /><Bar dataKey="billed" name="Facturé" fill={colors.indigo} radius={[7, 7, 0, 0]} /></BarChart></ResponsiveContainer></HrChartCard><HrSectionCard icon={CircleDollarSign} title="Synthèse financière et EVM" description="Indicateurs calculés depuis le journal mensuel, avec formules traçables et dénominateurs sécurisés."><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><HrInfo label="BAC" value={money(latestFinance.bac || project.ordered_budget || project.budget_amount)} accent="indigo" /><HrInfo label="CPI = VA / CR" value={Number(latestFinance.cpi || 0).toFixed(3)} accent={Number(latestFinance.cpi || 0) >= 1 ? "emerald" : "rose"} /><HrInfo label="SPI = VA / VP" value={Number(latestFinance.spi || 0).toFixed(3)} accent={Number(latestFinance.spi || 0) >= 1 ? "emerald" : "rose"} /><HrInfo label="EAC" value={money(latestFinance.estimate_at_completion)} accent="amber" /><HrInfo label="Écart coût = VA − CR" value={money(latestFinance.cost_variance)} /><HrInfo label="Écart délai = VA − VP" value={money(latestFinance.schedule_variance)} /><HrInfo label="FAE" value={money(latestFinance.fae)} accent="sky" /><HrInfo label="PCA" value={money(latestFinance.pca)} accent="amber" /></div></HrSectionCard><ProjectTable title="Historique satisfaction" description="Notes mensuelles par critère et actions d’amélioration client." icon={Activity} rows={data.satisfaction} columns={[{ label: "Mois", value: (row) => formatMonth(row.survey_month) }, { label: "Écoute", value: (row) => `${row.customer_listening_score}/5` }, { label: "Planification", value: (row) => `${row.planning_score}/5` }, { label: "Compétences", value: (row) => `${row.technical_skills_score}/5` }, { label: "Indicateurs", value: (row) => `${row.monitoring_score}/5` }, { label: "Risques", value: (row) => `${row.risk_management_score}/5` }, { label: "Global", value: (row) => `${Number(row.overall_score || 0).toFixed(1)}/5` }, { label: "Action", value: (row) => row.improvement_actions || "—" }]} /></div>}
