...
export default function ProjectSkillRequirementsForm({ organizationId, projectId, library, initialRows, onClose, onSaved }: { organizationId: string; projectId: string; library: AnyRow[]; initialRows: AnyRow[]; onClose: () => void; onSaved: () => void }) {
  const sortedLibrary = useMemo(() => library.slice().sort((a, b) => String(a.family || "").localeCompare(String(b.family || ""), "fr") || String(a.category || "").localeCompare(String(b.category || ""), "fr") || String(a.code || "").localeCompare(String(b.code || ""), "fr", { numeric: true })), [library]);
        ...
        <table className="w-full min-w-[1900px] border-separate border-spacing-0 text-sm">
          ...
          <tbody>{rows.map((row, index) => {
            ...
            return <tr key={row.id || index} className="hover:bg-indigo-50/35 dark:hover:bg-indigo-900/20">
              <td className="sticky left-0 z-30 w-[220px] border-b border-slate-100 bg-white px-3 py-2 dark:border-slate-600 dark:bg-slate-700"><select value={row.chapter} onChange={(event) => { const chapter = event.target.value; const subchapter = unique(sortedLibrary.filter((skill) => skill.family === chapter).map((skill) => String(skill.category || "")))[0] || ""; update(index, { chapter, subchapter, skill_id: "", skill_name: "" }); }} className={`${hrSelectClassName} w-full`}><option value="">Sélectionner</option>{chapters.map((chapter) => { const sample = sortedLibrary.find((skill) => skill.family === chapter); return <option key={chapter} value={chapter}>{sample?.chapter_code ? `${sample.chapter_code} · ` : ""}{chapter}</option>; })}</select></td>
              <td className="sticky left-[220px] z-30 w-[240px] border-b border-slate-100 bg-white px-3 py-2 dark:border-slate-600 dark:bg-slate-700"><select value={row.subchapter} onChange={(event) => update(index, { subchapter: event.target.value, skill_id: "", skill_name: "" })} className={`${hrSelectClassName} w-full`}><option value="">Sélectionner</option>{subchapters.map((subchapter) => { const sample = sortedLibrary.find((skill) => skill.family === row.chapter && skill.category === subchapter); return <option key={subchapter} value={subchapter}>{sample?.subchapter_code ? `${sample.subchapter_code} · ` : ""}{subchapter}</option>; })}</select></td>
              <td className="sticky left-[460px] z-30 w-[360px] border-b border-slate-100 bg-white px-3 py-2 dark:border-slate-600 dark:bg-slate-700"><select value={row.skill_id} onChange={(event) => { const selected = sortedLibrary.find((skill) => String(skill.id) === event.target.value); update(index, { skill_id: selected?.id || "", skill_name: selected?.name || "", skill_code: selected?.code || "" }); }} className={`${hrSelectClassName} w-full`}><option value="">Sélectionner une compétence</option>{skills.map((skill) => <option key={skill.id} value={skill.id}>{skill.code} · {skill.name}</option>)}</select></td>
              <td className="border-b border-slate-100 px-3 py-2"><select value={row.required_level} onChange={(event) => update(index, { required_level: Number(event.target.value) })} className={`${hrSelectClassName} w-36`}>{[0, 1, 2, 3, 4].map((level) => <option key={level} value={level}>N{level} / 4</option>)}</select></td>
