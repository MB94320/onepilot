...
export default function ProjectAuditArrow({ themes, questions, audits, responses }: { themes: AnyRow[]; questions: AnyRow[]; audits: AnyRow[]; responses: AnyRow[] }) {
  ...
  return <section ref={captureRef} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm fullscreen:overflow-auto">
    ...
    <div data-visual-scroll className="overflow-hidden bg-slate-50/60 p-4">
      <svg viewBox="0 0 1600 510" className="block h-auto w-full" role="img" aria-label="Flèche des thèmes de conformité depuis l’AVV vers le Delivery">
        ...
        <text x="180" y="482" textAnchor="middle" fontSize="20" fontWeight="900" fill="#0369a1">AVV · opportunité et engagement</text>
        <text x="1395" y="482" textAnchor="middle" fontSize="20" fontWeight="900" fill="#4f46e5">DELIVERY · exécution et qualité</text>
      </svg>
    </div>
    <div className="flex flex-wrap items-center justify-center gap-5 border-t border-slate-100 px-5 py-3 text-[10px] font-black"><span className="inline-flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-emerald-300" />Conforme ≥ 80 %</span><span className="inline-flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-amber-300" />Partiellement conforme 65–79 %</span><span className="inline-flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-rose-300" />Non conforme &lt; 65 %</span><span className="inline-flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-slate-300" />Non applicable</span><span className="text-slate-600">{latest ? `${latest.audit_number} · ${formatDate(latest.audit_date)} · ${Number(latest.overall_score || 0).toFixed(1)} %` : "Aucun audit réalisé"}</span></div>
  </section>;
}
