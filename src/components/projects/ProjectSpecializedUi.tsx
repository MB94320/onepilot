...
export function PerformancePilotage({ projects, onOpen }: { projects: AnyRow[]; onOpen: (row: AnyRow) => void }) {
  return (
    <div className="space-y-4">
      <div className="grid gap-4 xl:grid-cols-2">{projects.map((project) => {
        const health = calculateProjectHealth(project);
        const tone = healthTone(health.score, health.critical);
        return <button key={project.id} type="button" onClick={() => onOpen(project)} className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:border-indigo-200 hover:bg-indigo-50/25 dark:border-slate-600 dark:bg-slate-700/70 dark:hover:bg-indigo-900/20"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-black text-slate-950 dark:text-white">{project.code} · {project.name}</h3><p className="mt-1 text-xs text-slate-500 dark:text-slate-300">{project.client_name} · {project.manager_name}</p></div><HrStatusBadge status={tone.status} label={`${tone.label} · ${health.score}/100`} /></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><HrInfo label="TACE" value={percent(project.tace)} accent="indigo" /><HrInfo label="Satisfaction" value={`${Math.round(number(project.satisfaction_score) * 20)}/100`} accent="emerald" /><HrInfo label="CPI / SPI" value={`${number(project.cpi).toFixed(2)} / ${number(project.spi).toFixed(2)}`} accent="amber" /><HrInfo label="Marge" value={percent(number(project.margin_rate) * (Math.abs(number(project.margin_rate)) <= 1 ? 100 : 1))} accent="rose" /></div></button>;
      })}</div>
      <div className="max-h-[252px] overflow-auto rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-600 dark:bg-slate-700/70"><table className="min-w-[2500px] w-full text-xs"><thead className="sticky top-0 z-20 bg-slate-50 text-[10px] font-black uppercase text-slate-500 dark:bg-slate-600 dark:text-slate-200"><tr>{["Projet", "Client", "Responsable", "Période", "Santé", "Avancement physique", "VP", "VA", "CR", "BAC", "EAC", "ETC", "VAC", "CPI", "SPI", "TACE", "Livrables", "Livrables en retard", "OTD", "DoD", "OQD", "Satisfaction", "Risques critiques", "Charge prévue", "Charge réelle", "Production", "Coûts", "Marge", "Fiabilité", "Dernière mise à jour"].map((label, index) => <th key={label} className={`whitespace-nowrap px-3 py-3 text-left ${index === 0 ? "sticky left-0 z-30 bg-slate-50 dark:bg-slate-600" : ""}`}>{label}</th>)}</tr></thead><tbody>{projects.map((project) => { const health = calculateProjectHealth(project); const tone = healthTone(health.score, health.critical); const bac = number(project.bac || project.baseline_budget || project.ordered_budget); const eac = number(project.eac); const ac = number(project.actual_cost_total || project.consumed_budget); return <tr key={project.id} onClick={() => onOpen(project)} className="cursor-pointer hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20"><td className="sticky left-0 bg-white px-3 py-3 font-black text-indigo-700 dark:bg-slate-700 dark:text-indigo-200">{project.code} · {project.name}</td><td className="px-3 py-3">{project.client_name}</td><td className="px-3 py-3">{project.manager_name}</td><td className="px-3 py-3">{project.reporting_period || "Dernière période"}</td><td className="px-3 py-3"><HrStatusBadge status={tone.status} label={`${health.score}/100`} /></td><td className="px-3 py-3">{percent(project.progress_percent)}</td><td className="px-3 py-3">{money(project.planned_value)}</td><td className="px-3 py-3">{money(project.earned_value)}</td><td className="px-3 py-3">{money(ac)}</td><td className="px-3 py-3">{money(bac)}</td><td className="px-3 py-3">{money(eac)}</td><td className="px-3 py-3">{money(eac - ac)}</td><td className={`px-3 py-3 font-black ${bac - eac < 0 ? "text-rose-700" : "text-emerald-700"}`}>{money(bac - eac)}</td><td className="px-3 py-3">{number(project.cpi).toFixed(2)}</td><td className="px-3 py-3">{number(project.spi).toFixed(2)}</td><td className="px-3 py-3">{percent(project.tace)}</td><td className="px-3 py-3">{number(project.deliverable_count)}</td><td className="px-3 py-3">{number(project.late_deliverables)}</td><td className="px-3 py-3">{percent(project.otd)}</td><td className="px-3 py-3">{number(project.dod)} j</td><td className="px-3 py-3">{percent(project.oqd)}</td><td className="px-3 py-3">{Math.round(number(project.satisfaction_score) * 20)}/100</td><td className="px-3 py-3">{number(project.critical_risks)}</td><td className="px-3 py-3">{number(project.planned_hours)} h</td><td className="px-3 py-3">{number(project.actual_hours)} h</td><td className="px-3 py-3">{money(project.production_amount)}</td><td className="px-3 py-3">{money(ac)}</td><td className="px-3 py-3">{percent(number(project.margin_rate) * (Math.abs(number(project.margin_rate)) <= 1 ? 100 : 1))}</td><td className="px-3 py-3">{percent(project.reporting_reliability_percent || 60)}</td><td className="px-3 py-3">{formatDate(project.last_reporting_at || project.updated_at)}</td></tr>; })}</tbody></table></div>
    </div>
  );
}
...
const emptyTask = (): TaskDraft => ({ code: "", name: "", description: "", wbs: "", lot: "", owner: "", start: "", end: "", hours: "", progress: "0", predecessor: "", successor: "", milestone: false, risk: false, status: "todo", critical: false, comments: "" });

export function BulkTaskForm({ organizationId, projects, employees, onClose, onSaved }: { organizationId: string; projects: AnyRow[]; employees: AnyRow[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div className="max-h-[94vh] w-full max-w-[96vw] overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-600 dark:bg-slate-800"><div className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-sky-50 via-white to-indigo-50 px-5 py-4 dark:border-slate-600 dark:from-sky-900/25 dark:via-slate-800 dark:to-indigo-900/25"><div><h2 className="text-sm font-black text-slate-950 dark:text-white">Nouvelles tâches</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-300">Créer en série la WBS, les tâches, jalons, charges et dépendances d’un projet.</p></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-white dark:hover:bg-slate-700"><X className="h-4 w-4" /></button></div><div className="space-y-4 p-5"><div className="grid gap-4 sm:grid-cols-[minmax(320px,1fr)_220px]"><label><span className="text-xs font-bold text-slate-600 dark:text-slate-300">Projet</span><select value={projectId} onChange={(event) => setProjectId(event.target.value)} className={`${hrSelectClassName} mt-1 w-full`}>{projects.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label><HrInfo label="Statut projet" value={statusLabels[project?.status] || project?.status || "Non renseigné"} accent="sky" /></div><div className="overflow-auto rounded-2xl border border-slate-200 dark:border-slate-600"><table className="min-w-[2500px] w-full text-xs"><thead className="sticky top-0 z-20 bg-slate-50 text-[10px] font-black uppercase text-slate-500 dark:bg-slate-600 dark:text-slate-200"><tr>{["Code", "Nom de la tâche", "Description", "Niveau WBS", "Lot / livrable", "Responsable", "Début", "Fin", "Charge (h)", "Avancement", "Prédécesseurs", "Successeurs", "Jalon", "Risque", "Statut", "Critique", "Commentaires", "Actions"].map((label, index) => <th key={label} className={`px-2 py-3 text-left ${index === 0 ? "sticky left-0 z-30 bg-slate-50 dark:bg-slate-600" : ""}`}>{label}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={index}>{(["code", "name", "description", "wbs", "lot"] as const).map((key) => <td key={key} className={`${key === "code" ? "sticky left-0 z-10 bg-white dark:bg-slate-700" : ""} p-1.5`}><input value={row[key]} onChange={(event) => update(index, key, event.target.value)} className={`${hrInputClassName} min-w-[140px]`} /></td>)}<td className="p-1.5"><select value={row.owner} onChange={(event) => update(index, "owner", event.target.value)} className={`${hrSelectClassName} min-w-[180px]`}><option value="">Non affecté</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{person(employee)}</option>)}</select></td>{(["start", "end"] as const).map((key) => <td key={key} className="p-1.5"><input type="date" value={row[key]} onChange={(event) => update(index, key, event.target.value)} className={hrInputClassName} /></td>)}{(["hours", "progress"] as const).map((key) => <td key={key} className="p-1.5"><input type="number" min="0" max={key === "progress" ? 100 : undefined} value={row[key]} onChange={(event) => update(index, key, event.target.value)} className={`${hrInputClassName} w-24`} /></td>)}{(["predecessor", "successor"] as const).map((key) => <td key={key} className="p-1.5"><input value={row[key]} onChange={(event) => update(index, key, event.target.value)} placeholder="T-2026-0001" className={`${hrInputClassName} min-w-[140px]`} /></td>)}{(["milestone", "risk"] as const).map((key) => <td key={key} className="p-1.5 text-center"><input type="checkbox" checked={row[key]} onChange={(event) => update(index, key, event.target.checked)} /></td>)}<td className="p-1.5"><select value={row.status} onChange={(event) => update(index, "status", event.target.value)} className={`${hrSelectClassName} min-w-[130px]`}>{["todo", "in_progress", "blocked", "review", "done"].map((status) => <option key={status} value={status}>{statusLabels[status]}</option>)}</select></td><td className="p-1.5 text-center"><input type="checkbox" checked={row.critical} onChange={(event) => update(index, "critical", event.target.checked)} /></td><td className="p-1.5"><input value={row.comments} onChange={(event) => update(index, "comments", event.target.value)} className={`${hrInputClassName} min-w-[180px]`} /></td><td className="p-1.5"><button type="button" onClick={() => setRows((current) => current.filter((_, rowIndex) => rowIndex !== index))} className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-rose-600 hover:bg-rose-50"><Trash2 className="h-4 w-4" /></button></td></tr>)}</tbody></table></div><button type="button" onClick={() => setRows((current) => [...current, emptyTask()])} className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 py-2 text-xs font-bold text-indigo-700"><Plus className="h-4 w-4" />Ajouter une ligne</button></div><div className="flex justify-end gap-2 border-t border-slate-100 p-5 dark:border-slate-600"><button type="button" onClick={onClose} className="rounded-xl border border-rose-200 bg-white px-4 py-2.5 text-sm font-bold text-rose-700">Annuler</button><button type="button" onClick={() => void save()} disabled={saving || !projectId || !rows.some((row) => row.name.trim())} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{saving ? "Enregistrement…" : "Enregistrer les tâches"}</button></div></div></div>;
}
