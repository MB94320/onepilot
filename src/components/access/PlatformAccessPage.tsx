...
export default function PlatformAccessPage({ params }: { params: Promise<Params> }) {
  ...
  const [permissionMatrix, setPermissionMatrix] = useState<Record<string, "view" | "edit" | "admin">>({});
  ...
  const displayPermissions = (row: AnyRow) => {
    const selectedSubmodules: string[] = row.submodule_keys || [];
    if (!selectedSubmodules.length) return `Module complet · ${accessLabels[row.access_level] || row.access_level}`;
    return selectedSubmodules.map((submodule) => `${submodule} : ${accessLabels[row.permission_matrix?.[submodule] || row.access_level] || row.permission_matrix?.[submodule] || row.access_level}`).join(" · ");
  };
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        permission_matrix: Object.fromEntries(Object.entries(permissionMatrix).filter(([submodule]) => submodules.includes(submodule))),
  ...
  function resetForm() {
    setEditingId(null); setEmployeeId(""); setExternalName(""); setExternalEmail(""); setModuleKey("projects"); setSubmodules([]); setAccessLevel("view"); setPermissionMatrix({}); setCanExport(false); setCanShare(false); setEndsAt(""); setError("");
  }
  function openEdit(row: AnyRow) {
    setEditingId(row.id); setEmployeeId(row.employee_id || ""); setExternalName(row.external_name || ""); setExternalEmail(row.external_email || ""); setModuleKey(row.module_key); setSubmodules(row.submodule_keys || []); setAccessLevel(row.access_level); setPermissionMatrix(row.permission_matrix || {}); setCanExport(Boolean(row.can_export)); setCanShare(Boolean(row.can_share)); setEndsAt(row.ends_at ? String(row.ends_at).slice(0, 10) : ""); setError(""); setFormOpen(true);
  }
  ...
  const setSubmodulePermission = (submodule: string, enabled: boolean, level?: "view" | "edit" | "admin") => {
    setSubmodules((current) => enabled ? Array.from(new Set([...current, submodule])) : current.filter((value) => value !== submodule));
    setPermissionMatrix((current) => {
      const next = { ...current };
      if (!enabled) delete next[submodule]; else next[submodule] = level || next[submodule] || (accessLevel as "view" | "edit" | "admin");
      return next;
    });
  };
  const exportColumns: ExportColumn<AnyRow>[] = [
    ...
    { key: "submodules", label: "Droits par sous-module", value: displayPermissions },
  ...
  return <div className="onepilot-business-page space-y-6">
    ...
    {formOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/35 p-4" onMouseDown={(event) => { if (event.currentTarget === event.target) { setFormOpen(false); resetForm(); } }}><section className="max-h-[94vh] w-full max-w-6xl overflow-auto rounded-2xl border border-slate-200 bg-white shadow-2xl"><header className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-sky-50 via-white to-indigo-50 px-5 py-4"><div><h2 className="text-lg font-black text-slate-950">{editingId ? "Modifier l’accès" : "Nouvel accès"}</h2><p className="mt-1 text-xs text-slate-500">Un droit global par module et, si nécessaire, un niveau distinct pour chaque sous-module.</p></div><button type="button" onClick={() => { setFormOpen(false); resetForm(); }} className={hrCancelButtonClassName}>Fermer</button></header><div className="space-y-5 p-5"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><label><span className="text-xs font-bold text-slate-600">Ressource ONEPILOT</span><select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} className={`${hrSelectClassName} mt-1 w-full`}><option value="">Collaborateur externe ou module RH absent</option>{data.employees.map((employee: AnyRow) => <option key={employee.id} value={employee.id}>{employee.full_name || employee.employee_number}</option>)}</select></label><label><span className="text-xs font-bold text-slate-600">Nom externe</span><input value={externalName} onChange={(event) => setExternalName(event.target.value)} className={`${hrInputClassName} mt-1 w-full`} /></label><label><span className="text-xs font-bold text-slate-600">E-mail externe</span><input type="email" value={externalEmail} onChange={(event) => setExternalEmail(event.target.value)} className={`${hrInputClassName} mt-1 w-full`} /></label><label><span className="text-xs font-bold text-slate-600">Module</span><select value={moduleKey} onChange={(event) => { setModuleKey(event.target.value); setSubmodules([]); setPermissionMatrix({}); }} className={`${hrSelectClassName} mt-1 w-full`}>{modules.map((module) => <option key={module.key} value={module.key}>{module.label}</option>)}</select></label><label><span className="text-xs font-bold text-slate-600">Droit global par défaut</span><select value={accessLevel} onChange={(event) => setAccessLevel(event.target.value)} className={`${hrSelectClassName} mt-1 w-full`}><option value="view">Lecture</option><option value="edit">Modification</option><option value="admin">Administration</option></select></label><label><span className="text-xs font-bold text-slate-600">Fin de validité</span><input type="date" value={endsAt} onChange={(event) => setEndsAt(event.target.value)} className={`${hrInputClassName} mt-1 w-full`} /></label></div><HrSectionCard icon={KeyRound} title="Droits par sous-module" description="Cochez uniquement les sous-modules accessibles, puis choisissez Lecture, Modification ou Administration pour chacun."><div className="overflow-hidden rounded-2xl border border-slate-200"><table className={`${hrTableClassName} min-w-full`}><thead className={hrTableHeaderClassName}><tr><th className="w-28 text-left">Accès</th><th className="text-left">Sous-module</th><th className="w-64 text-left">Niveau autorisé</th></tr></thead><tbody>{availableSubmodules.map((submodule) => { const enabled = submodules.includes(submodule); return <tr key={submodule}><td><input type="checkbox" checked={enabled} onChange={(event) => setSubmodulePermission(submodule, event.target.checked)} className="h-4 w-4 accent-indigo-600" /></td><td className="font-normal text-slate-800">{submodule}</td><td><select disabled={!enabled} value={permissionMatrix[submodule] || accessLevel} onChange={(event) => setSubmodulePermission(submodule, true, event.target.value as "view" | "edit" | "admin")} className={`${hrSelectClassName} w-full disabled:cursor-not-allowed disabled:opacity-50`}><option value="view">Lecture</option><option value="edit">Modification</option><option value="admin">Administration</option></select></td></tr>; })}</tbody></table></div><p className="mt-3 text-xs leading-5 text-slate-500">Si aucun sous-module n’est coché, le droit global s’applique au module complet. Le niveau le plus élevé n’est jamais propagé automatiquement aux autres sous-modules.</p></HrSectionCard><div className="flex flex-wrap gap-3"><label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600"><input type="checkbox" checked={canExport} onChange={(event) => setCanExport(event.target.checked)} />Autoriser l’export</label><label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600"><input type="checkbox" checked={canShare} onChange={(event) => setCanShare(event.target.checked)} />Autoriser le partage</label></div>{error && <p className="rounded-xl border border-rose-100 bg-rose-50 p-3 text-sm font-bold text-rose-700">{error}</p>}<div className="flex justify-end gap-3"><button type="button" onClick={() => { setFormOpen(false); resetForm(); }} className={hrCancelButtonClassName}>Annuler</button><button type="button" onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending} className={hrSaveButtonClassName}>{saveMutation.isPending ? "Enregistrement…" : "Enregistrer"}</button></div></div></section></div>}
  </div>;
}
