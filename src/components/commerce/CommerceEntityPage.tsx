...
import { Activity, BarChart3, Building2, CalendarClock, CircleDollarSign, FileSearch, Kanban, List, PackageCheck, Plus, Search, SlidersHorizontal, Target, TrendingUp } from "lucide-react";
...
const config: Record<Mode, { title: string; subtitle: string; singular: string; icon: typeof Building2 }> = {
  ...
  prospects: { title: "Prospects & opportunités", subtitle: "Qualifier le pipeline, la probabilité, la valeur, la prochaine action et la décision commerciale.", singular: "opportunité", icon: Target },
...
async function loadCommerce(orgId: string, mode: Mode) {
  ...
  if (mode === "clients") return { organization, clients, rows: clients.map((row) => ({ ...row, display_status: row.archived_at ? "Archivé" : "Actif", display_date: row.updated_at || row.created_at, display_owner: row.contact || "Contact à renseigner" })) };
  ...
  const prospects: AnyRow[] = (prospectsResult.data || []).map((row: AnyRow) => ({
    ...row,
    opp_number: opportunityNumber(row.opp_number || row.opportunity_number, row.created_at),
    ...
    display_amount: Number(row["ca_estime_k€"] || row.montant || row.amount || 0),
    ...
    next_action: row.next_action || row.prochaine_action || "À planifier",
  }));
  ...
  if (mode === "offers") {
    ...
    const rows = offers.map((row) => {
      ...
      return { ...row, prospect, technical, goNoGo, opp_number: opportunityNumber(prospect?.opp_number || row.opportunity_number, row.created_at), title: prospect?.titre || row.title || "Offre sans désignation", client_name: prospect?.client_name || "Client à renseigner", display_status: translatedStatus(row.statut_offre || "À faire"), display_date: row.date_validation_previsionnelle || row.date_diffusion_previsionnelle || prospect?.display_date, display_amount: Number(technical?.total_prix_vente_ca || row.amount || 0), display_cost: Number(technical?.total_couts_ressources || 0) + Number(technical?.total_couts_achats || 0), display_margin: Number(technical?.marge_brute_calculee_pourcent || row.margin_percent || 0), display_gonogo: goNoGo?.date_passage_gonogo ? translatedStatus(goNoGo.decision_calculee || "À faire") : "À faire", display_owner: prospect?.display_owner || row.owner_name || "Non affecté" };
    });
...
export default function CommerceEntityPage({ params, mode }: { params: Promise<{ orgId: string }>; mode: Mode }) {
  ...
  const columns: Column[] = useMemo(() => {
    if (mode === "clients") return [column("name", "Client", (row) => row.name, (row) => row.name, "min-w-56"), column("sector", "Secteur", (row) => row.sector || "—", (row) => row.sector || "—"), column("country", "Pays", (row) => row.country || "—", (row) => row.country || "—"), column("city", "Ville", (row) => row.city || "—", (row) => row.city || "—"), column("contact", "Contact", (row) => row.contact || "—", (row) => row.contact || "—"), column("role", "Rôle", (row) => row.contact_role || "—", (row) => row.contact_role || "—"), column("email", "E-mail", (row) => row.email || "—", (row) => row.email || "—", "min-w-56"), statusColumn()];
    if (mode === "prospects") return [column("number", "N° Opportunité", (row) => row.opp_number, (row) => row.opp_number, "min-w-44"), column("title", "Désignation", (row) => row.titre || "—", (row) => row.titre || "—", "min-w-64"), column("client", "Client", (row) => row.client_name, (row) => row.client_name), column("owner", "Commercial", (row) => row.display_owner, (row) => row.display_owner), statusColumn("Étape"), column("date", "Décision prévue", (row) => date(row.display_date), (row) => date(row.display_date)), column("amount", "CA estimé", (row) => money(row.display_amount), (row) => row.display_amount), column("probability", "Probabilité", (row) => `${row.display_probability} %`, (row) => `${row.display_probability} %`), column("weighted", "CA pondéré", (row) => money(row.display_amount * row.display_probability / 100), (row) => row.display_amount * row.display_probability / 100), column("action", "Prochaine action", (row) => row.next_action, (row) => row.next_action, "min-w-56")];
    if (mode === "offers") return [column("number", "N° Opportunité", (row) => row.opp_number, (row) => row.opp_number, "min-w-44"), column("title", "Désignation", (row) => row.title, (row) => row.title, "min-w-64"), column("client", "Client", (row) => row.client_name, (row) => row.client_name), column("owner", "Responsable", (row) => row.display_owner, (row) => row.display_owner), column("gonogo", "Go/No-Go", (row) => <HrStatusBadge status={statusTone(row.display_gonogo)} label={row.display_gonogo} />, (row) => row.display_gonogo), statusColumn(), column("date", "Validation prévue", (row) => date(row.display_date), (row) => date(row.display_date)), column("amount", "Prix de vente", (row) => money(row.display_amount), (row) => row.display_amount), column("cost", "Coûts prévus", (row) => money(row.display_cost), (row) => row.display_cost), column("margin", "Marge prévue", (row) => `${row.display_margin.toFixed(1)} %`, (row) => `${row.display_margin.toFixed(1)} %`)];
    return [column("number", "N° Commande", (row) => row.num_commande || row.order_number || "—", (row) => row.num_commande || row.order_number || "—", "min-w-44"), column("opportunity", "N° Opportunité", (row) => row.opp_number, (row) => row.opp_number, "min-w-44"), column("title", "Désignation", (row) => row.title, (row) => row.title, "min-w-64"), column("entity", "Client / fournisseur", (row) => row.entity_name, (row) => row.entity_name), column("type", "Type", (row) => row.order_type, (row) => row.order_type), statusColumn(), column("date", "Date", (row) => date(row.display_date), (row) => date(row.display_date)), column("amount", "Montant", (row) => money(row.display_amount), (row) => row.display_amount)];
  }, [mode]);
  ...
  const openItem = (row: AnyRow) => { if (mode === "offers") router.push(`/${orgId}/avant-vente/${row.id}`); else if (mode === "orders") router.push(`/${orgId}/commandes/${row.id}`); else { setSelected(row); setFormOpen(true); } };
  const alerts = [
